...
const downloadPDF = (item) => {
  ...
  const title = "Job Order Schedule";
  ...
  // Start Date
  doc.setFont("helvetica", "bold");
  doc.text(20, 170, "Start Date:");
  doc.setFont("helvetica", "normal");
  doc.text(20, 175, new Date(item.startDate).toDateString());

  // End Date
  doc.setFont("helvetica", "bold");
  doc.text(20, 185, "End Date:");
  doc.setFont("helvetica", "normal");
  doc.text(20, 190, new Date(item.endDate).toDateString());

  // Save the PDF
  doc.save(`${item.firstName}_${item.lastName}_details.pdf`);
};

const InProgressInquiryDetails = ({ item, onClose }) => {
  ...
  return (
    <div className="relative mx-4 bg-white p-8 shadow-lg">
      ...
      <FormTitle>On Progress Details</FormTitle>
      ...
      <div className="max-h-[500px] min-w-[290px] overflow-y-auto bg-white md:max-h-[600px] md:min-w-[500px] md:max-w-[500px]">
        <div className="mb-4 flex flex-col text-sm md:text-base">
          ...
          {/* Start Date */}
          <span className="py-1 font-semibold">Start Date:</span>
          <p className="rounded-md bg-gray-200 p-2">
            {new Date(item.startDate).toDateString()}
          </p>
          {/* End Date */}
          <span className="py-1 font-semibold">End Date:</span>
          <p className="rounded-md bg-gray-200 p-2">
...
export default InProgressInquiryDetails;
